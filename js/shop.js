 ...
 * УСЛОВИЯ задачи:
 ...
 есть у тебя набор данных для интернет-магазина
 ...
 где USDRUB — цена бакса за рубли
 cart — корзина юзера, куда он накидал товаров
 storage — склад в фрмате { id: есть на складе }
 товары в корзине повторяются, но с разным количеством (типа id1 кол-во 2, потом id1 кол-во 5)
 их цена в USD. Тебе нужно:
 не используя циклы (совсем не используя), только с помощью методов map/filter/reduce/find/... сделать вот что:
 сгруппировать товары по id и сложить их количество, чтобы было id1 кол-во 7, id2 кол-во n и т.д.
 перевести цену из USD в RUB по курсу в переменной USDRUB
 проверить, есть ли на складе товар, если нет, то просто выбросить его из корзины и посчитать финальную цену корзину, чтобы её структура была такой:
 {
   total: { currency: валюта, price: цена },
   items: [ тут все item'ы ]
 }
 */
